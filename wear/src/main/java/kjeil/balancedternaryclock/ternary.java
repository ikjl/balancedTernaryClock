...
public class ternary extends CanvasWatchFaceService {
    ...
    // long z = 0;
    // long w = 0;
    // long v = 0; // count registering receiver
    // long u = 0; // count yesterdayTomorrow()
    // long t = 0; 
    // float floaty;
    // float floatz;
    // float floatx;
    ...
    private class Engine extends CanvasWatchFaceService.Engine {
        ...
        float inPuff = 1; // scale for interactive mode
        float amPuff = 1; // scale for ambient mode
        ...
        float [] stringRatio = {(float)1, (float)1/(float)2, (float).8, (float)1, (float).8, (float).8,
                                // -                             skull
                                (float)1, (float).6, (float).7, (float).5, (float).5, (float).7,
                                //rad                               gear 
                                (float).46, (float).5, (float).55, (float).4, (float).5, (float).5,
                                // umbrella                        chemistry
                                (float).36, (float).5, (float).3, (float).4, (float).5, (float).6,
                                // frown
                                (float).5, (float).25, (float).5, (float).4, (float).4, (float).4, (float)1};
        float [] vShiftRay = new float[31]; // yShift/2 - hiehgt / 2
        ...
        long [] minutes = new long [62]; 
        long [] seconds = new long [62];
        ...
        public void onCreate(SurfaceHolder holder) {
            ...
            for (c = 0 ; c < 62; c++) {
               minutes[c] = c * 60000 + (c > 29 ? (c == 30 ? 0 : -30000) : 30000);
            }
            for (c = 0 ; c < 62; c++){
               seconds[c] = c * 1000 + (c > 29 ? (c == 30 ? 0 : -500) : 500);
            }
        ...
        private void setTextArraySize(Paint[] array, float ts, boolean isCommon){
            for (int i = 0; i < array.length; i++) {
                array[i].setTextSize(isCommon ? ts : ts * stringRatio[i + stringRayOffset]);
            } 
        }
        ...
        public void onPeekCardPositionUpdate(Rect bounds) {
            ...
            if (!bounds.equals(mCardBounds)) {
                ...
                if (bounds.top < baseLedge || ledge != baseLedge) {
                    ...
                    inPuff = (ledge - dispTop - ledgeSpace) / mantle; 
                    if (inPuff != 1) {amPuff = (ledge - dispTop - ledgeSpace) / amMantle; } 
                    else {amPuff = 1;} 
                    // prevent over and under scalling.
                    if (amPuff > 1) {amPuff = 1;} 
                    else {if (amPuff < 0){amPuff = 9;}}
                    if (inPuff > 1){inPuff = 1;}
                    else { if (inPuff < (float)0) {inPuff =0; }}
                    // call resize method
                    puff = isInAmbientMode() ? amPuff : inPuff;
                    puffer(puff); 
           ...
           if (mAmbient != inAmbientMode) {
              ...
              if (mLowBitAmbient) {
                 ...
                 if (isInAmbientMode()){
                    ...
                    if (amPuff != puff) { puff = amPuff; puffer(puff);}
                 }
                 else {
                    ...
                    if (inPuff != puff) {puff = inPuff; puffer(puff);} 
                 }
              } 
           }
        ...
        public void onDraw(Canvas canvas, Rect bounds) {
            ...
            k = 0; for (byte i : timeray ) {
                ...
                canvas.drawText(stringRay[j + stringRayOffset], posX, posY 
                      + vShiftRay[j + stringRayOffset], textRay[j]);
                k += 1; posX += xShift;
            }
            ...
            for (byte i : wkDayTryte) {
                j = i + (byte)1;
                canvas.drawText(stringRay[j + stringRayOffset], posX, posY 
                      + smallVShiftRay[j + stringRayOffset], smallTextRay [j]);
                posX += xThird;
            }
            ...
            // if (greebo) {
            //     canvas.drawText(String.format("%1$d %2$d",
            //             v, u)
        ...
        private void updateTime(){
            ...
            if ( now.getTimeInMillis() >= tomorrow.getTimeInMillis() || 
                 now.getTimeInMillis() <= yesterday.getTimeInMillis()){
                yesRefresh = true; 
                yesterdayTomorrow();
                now.setTimeInMillis(System.currentTimeMillis());
            }
        ...
        private void minTrits (boolean yesRefresh) {
            if (yesRefresh || time >= nextMinute){
               ...
               nextMinute = nextHour - (c < 31 ? (long)1800000 : (long)5400000) + minutes[c];
               // i is for item
               k = 3; 
               for (int i : trits4) {
                   if (timeSlate > ((float) i / 2)) {
